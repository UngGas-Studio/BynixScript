...
function executeCode(code) {
  ...
  code = code.replace(/print(\.[a-zA-Z]+)?\((.+?)\)/g, (match, p1, p2) => {
    ...
    if (type.includes(".info") || type.includes(".warn") || type.includes(".debug") || type.includes(".trace") || type.includes(".assert")) {
  ...
  code = code.replace(/is(\_[a-zA-Z]+)?\s*===\s*(["'`])(.*?)\1/g, (match, p1, p2) => {
    const type = p1 || '';
    if (type.includes("_example11")) {
      return `// example11(${p2})`;
    }
  })
